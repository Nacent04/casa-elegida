...
app.post('/subir-logo', upload.single('logo'), async (req, res) => { if (!req.file) return res.status(400).json({ error: 'No imagen' }); const r = await cloudinary.uploader.upload(req.file.path, { folder: 'casa-elegida' }); fs.unlinkSync(req.file.path); setConfig('logo', r.secure_url); res.json({ success: true }); });
...
app.post('/guardar-categoria', (req, res) => { const { id, nombre, subcategorias } = req.body; if (!nombre?.trim()) return res.status(400).json({ error: 'Nombre requerido' }); if (db.prepare('SELECT id FROM categorias WHERE id=?').get(id)) db.prepare('UPDATE categorias SET nombre=?,subcategorias=? WHERE id=?').run(nombre.trim(), JSON.stringify(subcategorias||[]), id); else db.prepare('INSERT INTO categorias (id,nombre,subcategorias) VALUES (?,?,?)').run(id||Date.now(), nombre.trim(), JSON.stringify(subcategorias||[])); res.json({ success: true }); });
...
app.post('/tienda/marcar-abonado', (req, res) => { db.prepare("UPDATE pedidos SET estado='abonado' WHERE id=?").run(req.body.pedidoId); res.json({ success: true }); });
app.post('/tienda/marcar-enviado', (req, res) => { db.prepare("UPDATE pedidos SET estado='enviado' WHERE id=?").run(req.body.pedidoId); res.json({ success: true }); });
...
app.post('/tienda/retirar-pedido', (req, res) => { const p = db.prepare('SELECT * FROM pedidos WHERE id=? AND pin=?').get(req.body.pedidoId, req.body.pin); if (!p) return res.status(400).json({ error: 'PIN incorrecto' }); db.prepare("UPDATE pedidos SET estado='entregado' WHERE id=?").run(p.id); res.json({ success: true }); });
...
app.post('/admin/apertura-caja', adminMiddleware('ventas'), (req, res) => { db.prepare("INSERT OR REPLACE INTO caja_diaria (fecha, montoInicial, abiertaPor, estado, aperturaTimestamp) VALUES (?, ?, ?, 'abierta', ?)").run(new Date().toLocaleDateString('es-AR'), req.body.montoInicial||0, req.admin.nombre, Date.now()); res.json({ success: true }); });
app.post('/admin/cierre-caja', adminMiddleware('ventas'), (req, res) => { const hoy = new Date().toLocaleDateString('es-AR'); const v = db.prepare("SELECT COALESCE(SUM(total),0) as t, COUNT(*) as c FROM ventas WHERE date(fecha) = date('now','localtime')").get(); db.prepare("UPDATE caja_diaria SET estado='cerrada', totalVentas=?, totalEsperado=? WHERE fecha=? AND estado='abierta'").run(v.t, (db.prepare('SELECT montoInicial FROM caja_diaria WHERE fecha=?').get(hoy)?.montoInicial||0)+v.t, hoy); res.json({ success: true, resumen: { totalVentas: v.t, cantidadVentas: v.c } }); });
app.post('/admin/estado-caja', adminMiddleware('ventas'), (req, res) => { const hoy = new Date().toLocaleDateString('es-AR'); const c = db.prepare('SELECT * FROM caja_diaria WHERE fecha=?').get(hoy); const v = db.prepare("SELECT COALESCE(SUM(total),0) as t FROM ventas WHERE date(fecha) = date('now','localtime')").get(); res.json(c ? { abierta: c.estado==='abierta', montoInicial: c.montoInicial, totalVentas: v.t } : { abierta: false }); });
